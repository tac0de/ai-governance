...
- 신규 Core MCP는 `reports/governance/mcp-request-*.json`로 승인 흐름을 남긴다.
...
- callback 실패율 상승: fallback 정책 활성화 후 원인 분석.
- allowlist/registry 불일치: 요청 상태를 `requested`로 되돌리고 재검토.
